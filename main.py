...
def scrape_exam_mate(year_list, category=3, subject=13):  # cat 3: IG, subject 11: Maths
    for year in year_list:
        for season in ["summer", "winter"]:
            for time_zone in [1, 2, 3]:
                for paper in [1, 2]:  # create algorithm to find how many papers
                    for page_number in range(0, 100):
                        paper = scraper.ExamMatePaper(category=category, subject=subject, year=year, season=season,
...
def scrape_pdf_paper(category, subject_code, years, mark_scheme=False):
    ...
    seasons = ["summer"]
    time_zones = ["1"]
    past_paper = scraper.PDFPaper(category=category, subject_code=subject_code)
